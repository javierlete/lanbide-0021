...
--
/*!50003 DROP FUNCTION IF EXISTS `corregirNDIEmp` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `corregirNDIEmp`(nDIEmp VARCHAR(12)) RETURNS int
    NO SQL
BEGIN
RETURN CAST(REPLACE(nDIEmp, '.', '') AS UNSIGNED);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50001 VIEW `empleados` AS select `CORREGIRNDIEMP`(`empleado`.`nDIEmp`) AS `id`,`empleado`.`nomEmp` AS `nombre`,`empleado`.`sexEmp` AS `sexo`,`empleado`.`fecNac` AS `fecha_nacimiento`,`empleado`.`fecIncorporacion` AS `fecha_incorporacion`,`empleado`.`salEmp` AS `salario`,`empleado`.`comisionE` AS `comision`,`empleado`.`cargoE` AS `cargo`,`empleado`.`jefeID` AS `jefes_id`,`empleado`.`codDepto` AS `departamentos_id` from `empleado` */;
...
-- Dump completed on 2022-02-11 11:38:14
